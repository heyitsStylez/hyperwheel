...
| `05b-pnl.js` | 48 | `computePnl(trades, assetFilter)` → `{ realised }`. Cash-flow-lens P&L calculator. Slice 1: Realised path only — `Σ settled netPrem + Σ (strike − costBasis) × calledSize`. Open options + open lots contribute zero; HOLDING-originated and ASSIGNED-originated lots realise symmetrically. Pure; dual-exported. ADR: `docs/adr/0001-pnl-cash-flow-lens.md`. Unrealised + Total land in later slices |
...
| `07-render-charts.js` | 640 | `setCpnlPeriod` (1M/3M/ALL), `rCpnlChart` (cumulative premium hero + Realised P&L sparkline), `rCharts` (Premium P&L total/monthly tabs — Total tab consumes `computePnl` for the Realised tile), `cOpts` (Chart.js options factory) |
