...
| `05b-pnl.js` | 65 | `computePnl(trades, assetFilter)` → `{ realised, realisedSeries }`. Cash-flow-lens P&L calculator. Slice 1: Realised path only — `Σ settled netPrem + Σ (strike − costBasis) × calledSize`. `realisedSeries` is sorted cumulative `{date,val}` powering the hero sparkline. Open options + open lots contribute zero; HOLDING-originated and ASSIGNED-originated lots realise symmetrically. Pure; dual-exported. ADR: `docs/adr/0003-pnl-cash-flow-lens.md`. Unrealised + Total land in later slices |
