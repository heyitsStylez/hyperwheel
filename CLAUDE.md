...
| `02-utils.js` | 33 | `today()`, `save()` (routes trades through the persistence seam `persist()`), `fmt()` (max 2dp), `sk()` (K-abbrev), `capturePct(premium, closeCost)` (% of premium kept on buy-to-close; `null` when `premium` is 0/undefined — #124), `loadWallet()`, `saveWallet()`, `toast(msg, kind?)` (`'ok'`/`'err'`/`'info'`). **TradFi contract seam:** `SHARES_PER_CONTRACT` (100), `contractsToShares`/`sharesToContracts` — the single place the ×100 lives (#91); the lot engine works purely in shares. Dual-exports `today/fmt/sk` + the contract helpers for Node tests |
