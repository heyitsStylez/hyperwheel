...
| `01b-asset-meta.js` | 24 | `assetColor(sym)` (brand override → stable symbol-hash HSL fallback), `minSize(sym)` (contract minimum, `0` for unknown tickers), and `contractMultiplier(sym)` (TradFi ×N; default 100, futures override e.g. MES=5). Backed by `ASSET_BRAND`/`ASSET_MIN_SIZE`/`ASSET_MULTIPLIER` registries populated by platform modules. Dual-exported. Lets arbitrary tickers work with no config |
| `02-utils.js` | 33 | `today()`, `save()` (routes trades through the persistence seam `persist()`), `fmt()` (max 2dp), `sk()` (K-abbrev), `capturePct(premium, closeCost)` (% of premium kept on buy-to-close; `null` when `premium` is 0/undefined — #124), `daysEarly(expiry, closeDate)` (days left on the clock at buy-to-close = `expiry − closeDate`; `null` if either missing — #124), `loadWallet()`, `saveWallet()`, `toast(msg, kind?)` (`'ok'`/`'err'`/`'info'`). **TradFi contract seam:** `SHARES_PER_CONTRACT` (100, the equity default), `contractsToShares(contracts, asset)`/`sharesToContracts(shares, asset)` — the single place the ×N lives (#91); the multiplier is per-asset via `contractMultiplier(asset)` (01b): 100 for equities/ETFs, the point multiplier for futures (MES = 5). `entryMultiplier(asset, isHolding)` is the entry→stored rule (×1 for equity holdings, ×multiplier for options + futures holdings). The lot engine works purely in shares. Dual-exports `today/fmt/sk` + the contract helpers for Node tests |
...
| `13-edit-modal.js` | ~120 | `openEditModal(id, presetOutcome?)`, `closeEditModal`, `saveEdit`, `toggleEditCloseCost` — fields prefixed `ef-` (NOT `e-` despite older docs). Outcome select includes CLOSED; a `ef-closecost` field (in `#ef-closecost-field`) shows only when CLOSED is selected and is netted off premium. `presetOutcome` lets the row Close button open the modal straight into the buy-to-close flow. On Wheeler (tradfi), option and futures-holding `size` round-trips through `entryMultiplier` (labelled **Contracts**); equity/ETF holdings and crypto edit in raw size (#91) |
...
| File | Lines | Key exports / purpose |
|------|------:|-----------------------|
| `11a-futures-meta.js` | 6 | Registers futures contract multipliers as overrides on the core `ASSET_MULTIPLIER` registry (MES=5). Mirrors crypto's `11a-asset-brand.js`. Add ES/MNQ/M2K here as needed. Runs at load after `01b-asset-meta.js` |
...
| `12-cloud-sync.js` | ~95 | **Wheeler cloud sync** (#110, ADR 0007, Design B): `authInit` (boot: GET `/api/wheeler-sync` → set gate UI, then **union-merge** local+remote via `mergeTradesById` and converge whichever store is behind — never wholesale-replaces, so a pull can't drop a local-only trade), `mergeTradesById(local, remote, preferRemote)` (pure, dual-exported: union by `id`, newer side wins shared ids; deletes don't propagate), `cloudPush`/`scheduleCloudPush` (debounced POST of the FULL trade array), `wheelerSignIn`/`wheelerSignOut`, `_setAuthUI`, `_setCloudStatus`. Auth state via session cookie; only pushes when signed in |
...
| `20-ditm-calc.js` | ~120 | **DITM call calculator** (Wheeler-only, header `[ DITM CALC ]` button → `openDitmCalc()`). What-if modal (`#ditm-overlay` in shared `modals.html`) modelling a long deep-ITM call as a stock substitute vs. buying 100 shares — mirrors sheet 1 of `ditm_call_tracker.xlsx`. Inputs price/strike/premium/expiry → break-even (`strike+prem`), debit/max-loss, cost-of-100-shares, DTE tiles + an auto-generated price-level outcomes matrix (`ditmLevels`). Pure scratchpad — **nothing is saved to `trades`**. Own Esc/click-outside handlers |
| `19-tradfi-form.js` | ~50 | `setTicker(v)` (mirrors free-text ticker → `sAsset`, uppercased) and `wheelerAddTrade()` — manual PUT/CALL/HOLDING entry (crypto's core `addTrade()` is HOLDING-only), routed through `save()`/`render()`. Trades carry `platform: 'MANUAL'`. Supports the CLOSED (buy-to-close) outcome: reads `f-closecost` and stores `closeCost` (netted off premium by the core cash-flow lens). **Options are entered in contracts and stored as shares** via `entryMultiplier(asset, isHolding)` — per-asset multiplier, so MES stores contracts×5. Equity/ETF holdings enter in raw shares (×1); futures holdings enter in contracts too (×multiplier), so a manual holding matches an assigned-put lot. `refreshSizeLabel` flips the field label Shares↔Contracts by ticker+type. `setSizeDisplay(unit)` flips the header contracts↔shares toggle and re-renders. Display of `size` routes through `fmtSize`/`nativeSize` in `06-render-table.js`; futures render as contracts in both toggle states (no share concept) (#91) |
