...
| `13-edit-modal.js` | ~120 | `openEditModal(id, presetOutcome?)`, `closeEditModal`, `saveEdit`, `toggleEditCloseCost` — fields prefixed `ef-` (NOT `e-` despite older docs). Outcome select includes CLOSED; a `ef-closecost` field (in `#ef-closecost-field`) shows only when CLOSED is selected and is netted off premium. `presetOutcome` lets the row Close button open the modal straight into the buy-to-close flow |
